...
/**
 * Test class for both GradeCalculator.java and GradeCalculatorClass.java
 * This test captures the output from both classes and compares them to the expected output
 * Handles compilation errors gracefully and provides a final score
 * 
 * To run this test:
 * 1. javac GradeCalculator.java GradeCalculatorClass.java GradeCalculatorTest.java
...
public class GradeCalculatorTest {
    ...
    public static void main(String[] args) {
        ...
        // Test both classes
        TestResult result1 = testGradeCalculatorClass("GradeCalculator", expectedOutput);
        TestResult result2 = testGradeCalculatorClass("GradeCalculatorClass", expectedOutput);
        
        // Print individual results
        printTestResult("GradeCalculator", result1, expectedOutput);
        printTestResult("GradeCalculatorClass", result2, expectedOutput);
        
        // Calculate and print final score
        printFinalScore(result1, result2);
    }
     ...
     * @param result1 Result for GradeCalculator
     * @param result2 Result for GradeCalculatorClass
     */
    private static void printFinalScore(TestResult result1, TestResult result2) {
        int gradeCalculatorScore = result1.success ? 50 : 0;
        int GradeCalculatorClassScore = result2.success ? 50 : 0;
        int totalScore = gradeCalculatorScore + GradeCalculatorClassScore;
        
        System.out.println("\nSCORE: " + totalScore + "/100 (" + totalScore + "%)");
        
        // Show what functionality is working
        System.out.println("Functionality Status:");
        System.out.println("  GradeCalculator: " + (result1.success ? "Working" : "Not Working"));
        System.out.println("  GradeCalculatorClass: " + (result2.success ? "Working" : "Not Working"));
        
        // Show errors and fixes
        if (!result1.success) {
            System.out.println("\nGradeCalculator Error: " + 
                (result1.errorMessage != null ? result1.errorMessage : "Output doesn't match expected"));
            System.out.println("Recommended Fix: " + getRecommendedFix(result1));
        }
        
        if (!result2.success) {
            System.out.println("\nGradeCalculatorClass Error: " + 
                (result2.errorMessage != null ? result2.errorMessage : "Output doesn't match expected"));
            System.out.println("Recommended Fix: " + getRecommendedFix(result2));
        }
    }
